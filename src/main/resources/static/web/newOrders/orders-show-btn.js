...
function submitRetreat() {
    ...
    var surplusBond = parseFloat($(".surplusBond").html());
    var html = '<p>需要退还金额：<b style="color:#F00; font-size:18px;">'+(totalMoney+surplusBond)+'</b> 元<span style="color:#999;">（包含订单总额和压金金额）</span></p>'+
                '<input name="reason" type="text" class="form-control" placeholder="请输入退票原因" />';
    var retreatDialog = confirmDialog(html, "<i class='fa fa-info-circle'></i> 退票提示", function() {
        ...
        if($.trim(reason)=='') {showDialog("请认真输入退票原因", "<i class='fa fa-info-circle'></i> 系统提示");}
            ...
            $.post("/web/newOrders/retreatOrders", {no:ordersNo, reason:reason}, function(res) {
                alert(res.msg);
                if(res.code=='0') {
...
function submitCancel() {
    ...
    var retreatDialog = confirmDialog(html, "<i class='fa fa-info-circle'></i> 退票提示", function() {
        ...
        if($.trim(reason)=='') {showDialog("请认真输入取消订单的原因", "<i class='fa fa-info-circle'></i> 系统提示");}
            ...
            $.post("/web/newOrders/cancelOrders", {no:ordersNo, reason:reason}, function(res) {
                alert(res.msg);
                if(res.code=='0') {
                    window.location.reload();
                }
